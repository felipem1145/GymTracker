    ...
    <Transition name="sheet">
      ...
      >
        ...
        <div class="flex flex-col gap-3">
          <button
            @click="$emit('confirm')"
            class="w-full py-4 bg-red-600 hover:bg-red-700 active:scale-[0.98] text-white font-semibold rounded-2xl transition-all"
          >
            {{ confirmLabel }}
          </button>
          <button
            @click="$emit('cancel')"
            class="w-full py-4 bg-secondary hover:bg-secondary/80 active:scale-[0.98] text-secondary-foreground font-medium rounded-2xl transition-all"
          >
            {{ cancelLabel }}
          </button>
        </div>
